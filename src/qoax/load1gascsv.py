        ...
        for file in os.listdir(folder):
            if 'gases' in file and file.endswith('.csv'):
                ...
                df = pd.read_csv(folder+file, \
                                ...
                                names=['val.NO2.weV','val.NO2.cnc','val.NO2.aeV','val.NO2.weC','val.sht.hmd', \
                                       'val.sht.tmp','rec','tag'], \
                                #parse_dates=['rec']
                                )
        ...
        for file in os.listdir(folder):
            if 'status' in file and file.endswith('.csv'):
                ...
                df = pd.read_csv(folder+file, \
                                ...
                                'val.sch.scs-status.tally','val.sch.scs-status.interval','val.sch.scs-climate.tally', \
                                'val.sch.scs-climate.interval','val.sch.scs-gases.tally','val.sch.scs-gases.interval', \
                                'val.gps.pos:0','val.gps.pos:1','val.gps.elv','val.gps.qual','val.up.load.av15', \
                                'val.up.load.av1','val.up.load.av5','val.up.period','val.up.users','val.psu.prot-batt', \
                                'val.psu.rst','val.psu.chg','val.psu.standby','val.psu.pwr-in','val.psu.host-3v3', \
                                'val.psu.batt-flt','val.tz.name','val.tz.utc-offset','val.tmp.brd','rec','tag'],
                                #parse_dates=['rec']
                                )
                df.columns = map(str.lower, df.columns)
                df['rec'] = pd.to_datetime(df['rec'],errors='raise',utc=True, \
