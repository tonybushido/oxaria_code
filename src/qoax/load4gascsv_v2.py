...
def load4gascsv_v2(folder, topic):
    ...
    if 'gases' in topic:
        for file in os.listdir(folder):
            if 'gases' in file and file.endswith('.csv'):
                #if any(s in file for s in gas_flag) and file.endswith('.csv'):
                print(folder+file)
                ...
                df['rec'] = pd.to_datetime(df['rec'],errors='coerce',utc=True, \
                               unit='ns',origin='unix', yearfirst=True)  #infer_datetime_format=True,
        ...
        df['insert_date'] = dt.datetime.now(dt.timezone.utc).strftime("%d/%m/%Y %H:%M:%S")
    ...
    if 'particulate' in topic:
        for file in os.listdir(folder):
            ...
            #if any(s in file for s in pm_flag) and file.endswith('.csv'):
                print(folder+file)
                df = pd.read_csv(folder+file, \
                                low_memory=False, \
                                compression='infer', \
    ...
    if 'status' in topic:
        for file in os.listdir(folder):
            if 'status' in file and file.endswith('.csv'):
                #if any(s in file for s in stat_flag) and file.endswith('.csv'):
                print(folder+file)
                df = pd.read_csv(folder+file, \
                                low_memory=False, \
                                compression='infer', \
    ...
    if 'climate' in topic:
        for file in os.listdir(folder):
            if 'climate' in file and file.endswith('.csv'):
                #if any(s in file for s in clim_flag) and file.endswith('.csv'):
                print(folder+file)
                df = pd.read_csv(folder+file, \
                                low_memory=False, \
                                compression='infer', \
