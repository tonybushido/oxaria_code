...
| **NO2** |  |
| All time periods | [5_resample_stable_dfs_v2_with_transients_no2.ipynb](https://github.com/tonybushido/oxaria_code/blob/main/src/3_oct2021/5_resample_stable_dfs_v2_with_transients_no2.ipynb) |
...
Machine learnng methods developed for the handling & correction of environmental interferences acting upon NO2 & PM sensor readings are presented below. These were developed & implemented for 2 periods;

1. **Jan - Dec 2020**, to suport the preparation of the "methods paper" which set out a general approach for correction of sensor observations environmental interferences, investigating;

   > field “baselining” and interference correction using random forest regression methods for low-cost sensing of NO2, PM10 (particulate matter) and PM2.5.

    see https://amt.copernicus.org/articles/15/3261/2022/amt-15-3261-2022-discussion.html.

2. **Jan 2020 - Oct 2021**, to support the preparaton of the 2nd paper evaluating the imopacts of lockdown restriction on AQ using sensor data.

   > changes in PM (PM10 and PM2.5) concentrations for successive phases of COVID-19 restrictions using a low-cost sensor network in Oxford, UK.  Comparison of changes in PM occurring at multiple locations in this real world natural experimental study will inform prospective city-level air quality interventions and therefore present opportunities to benefit human health.

   In preparation.

## Methods paper Random Forest Regression modelling
This workflow item to be implemented after TASK 9 & before deployment (TASKs 10+). It utulises the baselined datasets generated by TASK 9 to traing the models using co-located sensor & reference methods obervations at 2 locations in Oxord (St Ebbes & High St).
...
This workflow item to be implemented after TASK 9 & before model deployment (TASKs 10+). It utulises the baselined datasets generated by TASK 9 to traing the models using co-located sensor & reference methods obervations at 2 locations in Oxord (St Ebbes & High St).

NB. the models developed for this ltime period focus exclusively on the corrected of PM10 & PM2.5 sensor observations.
